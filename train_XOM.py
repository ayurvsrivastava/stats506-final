...
if not saved_model:
    ...
    for epoch in range(500):
        ...
        for i in range(len(X_train)):
            ...
            loss.backward()
            optimizer.step()
        print(f"Epoch {epoch}, average loss: {np.mean(losses)}, lowest loss in epoch: {min(epoch_loss)}")
    ...
    torch.save(model.state_dict(), 'xom.pt')
    torch.save(optimizer.state_dict(), 'xom_opt.pt')
else:
